...
|--------|--------------|-----------|
| Skills | `(f:Company:ServiceFirm)-[:HAS_SKILL]->(s:Skill)` | `HAS_SKILL` |
| Categories | `(f:Company:ServiceFirm)-[:IN_CATEGORY]->(c:Category)` | `IN_CATEGORY` |
| Industries | `(f:Company:ServiceFirm)-[:SERVES_INDUSTRY]->(i:Industry)` | `SERVES_INDUSTRY` |
| Markets | `(f:Company:ServiceFirm)-[:OPERATES_IN]->(m:Market)` | `OPERATES_IN` |

**Node model note:** Platform member firms are `[:Company:ServiceFirm]` multi-label nodes — `Company` is the canonical base, `ServiceFirm` is a role label. Never query `MATCH (f:ServiceFirm)` alone — always include the `Company` label.
...
**Current status: LIVE.** pgvector extension enabled via migration `0003_pgvector_embeddings.sql`. The `embedding vector(1536)` column is active on `abstraction_profiles`. Cosine similarity is used when an embedding exists; text-overlap fallback applies for profiles not yet backfilled.

**What works:**
- `generateQueryEmbedding(queryText)` -- calls OpenAI text-embedding-3-small, returns `number[]`
- `vectorRerank(candidates, rawQuery, topK)` -- generates query embedding, then uses pgvector `<=>` cosine distance for profiles that have embeddings; text-overlap fallback for those without
- `generateFirmAbstraction()` -- now generates and stores embedding immediately after profile creation
- Backfill endpoint `POST /api/admin/abstractions/backfill` -- embeds existing profiles missing embeddings (50 per call, idempotent)

**Embedding pipeline:**
1. Firm abstraction profile generated by `generateFirmAbstraction()`
2. `hiddenNarrative` embedded via OpenAI `text-embedding-3-small`
3. Stored in `abstraction_profiles.embedding` (vector(1536))
4. HNSW index `abstraction_profiles_embedding_hnsw` enables fast cosine search
