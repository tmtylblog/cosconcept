...
async function condenseSummary(summary: string, roleTitle: string, companyName: string, specialistTitle?: string): Promise<string> {
  if (!summary) return "";
  try {
...
export function SpecialistProfileEditor({
  ...
  useEffect(() => {
    if (!showDescription || bodyDescription || hasAutoGenerated.current) return;
    if (!title.trim() || examples.length < 3) return;
    hasAutoGenerated.current = true;

    // Inline fetch to avoid stale closure
    setGenerating(true);
    fetch(`/api/experts/${expertId}/specialist-profiles/generate-description`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        title,
        examples: examples.map((ex) => ({
          ...
          companyName: ex.companyName, companyIndustry: ex.companyIndustry,
        })),
        bio: expertBio,
      }),
    })
      .then((r) => r.ok ? r.json() : null)
      .then((data) => { if (data?.description) setBodyDescription(data.description); })
      .catch(() => {})
      .finally(() => setGenerating(false));
  }, [showDescription, bodyDescription, title, examples, expertId, expertBio]);
