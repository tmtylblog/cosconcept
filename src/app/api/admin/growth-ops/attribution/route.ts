...
export async function GET() {
  ...
  const rows = await db
    .select({
      ...
      hasCompanyLinkedinMatch: attributionEvents.hasCompanyLinkedinMatch,
      companyLinkedinDetails: attributionEvents.companyLinkedinDetails,
      hasNameFuzzyMatch: attributionEvents.hasNameFuzzyMatch,
      nameFuzzyDetails: attributionEvents.nameFuzzyDetails,
      pdlLookupStatus: attributionEvents.pdlLookupStatus,
  ...
  const enrichedRows = rows.map((row) => {
    ...
    return {
      ...
      hasCompanyLinkedinMatch: row.hasCompanyLinkedinMatch,
      companyLinkedinDetails: row.companyLinkedinDetails,
      hasNameFuzzyMatch: row.hasNameFuzzyMatch,
      nameFuzzyDetails: row.nameFuzzyDetails,
      pdlLookupStatus: row.pdlLookupStatus,
  ...
  const byChannel = {
    ...
    companyLinkedin: enrichedRows.filter((r) => r.hasCompanyLinkedinMatch).length,
    nameFuzzy: enrichedRows.filter((r) => r.hasNameFuzzyMatch).length,
    direct: enrichedRows.filter((r) => r.matchMethod === "email_exact" || r.matchMethod === "name_domain").length,
    unattributed: enrichedRows.filter((r) => r.matchMethod === "none" && !r.hasLinkedinOrganic && !r.hasLinkedinCampaign && !r.hasCompanyLinkedinMatch).length,
  };
